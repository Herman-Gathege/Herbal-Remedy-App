...
function ProductsPage() {
  ...
  const handleAddHerb = () => {
    setCurrentHerb({ name: '', advantages: [], image: '', comments: [] }); // Change 'description' to 'advantages'
    setShowAddModal(true);
  };
  ...
  return (
    <div className="products-page">
      ...
      {showEditModal && (
        <div className="modal">
          <div className="modal-content">
            ...
            />
            <textarea
              name="advantages" // Change 'description' to 'advantages'
              value={currentHerb.advantages.join('\n')} // Display advantages as a multi-line string
              onChange={(e) => setCurrentHerb({ ...currentHerb, advantages: e.target.value.split('\n') })} // Split on new lines
              placeholder="Herb Advantages (one per line)"
            />
            <button onClick={handleSaveEdit}>Save</button>
            <button onClick={() => setShowEditModal(false)}>Cancel</button>
          </div>
      ...
      {showDeleteModal && (
        <div className="modal">
          <div className="modal-content">
            <h2>Are you sure you want to delete {currentHerb.name}?</h2>
            <button onClick={() => handleDelete(currentHerb.id)}>Yes</button>
      ...
      {showCommentModal && (
        <div className="modal">
          <div className="modal-content">
            <h2>Add Comment to {currentHerb.name}</h2>
            <textarea
              value={newComment}
              onChange={(e) => setNewComment(e.target.value)}
              placeholder="Your Comment"
            />
            <button onClick={() => handleAddComment(currentHerb.id)}>Submit Comment</button>
            <button onClick={() => setShowCommentModal(false)}>Cancel</button>
          </div>
        </div>
      )}
